...
const barraAltura = 0.4;  // Altura total da barra (0.2 para cima + 0.2 para baixo)
const barraLargura = 0.1; // Largura da barra
const velocidadeBarra = 0.05; // Velocidade de movimento das barras
const velocidadeBola = 0.01; // Mudança no offset da bola
...
function verificaColisaoBarraEsquerda() {
    // Posição da barra esquerda
    const barraX = -0.9;
    const barraYMin = tyBE - barraAltura / 2;
    const barraYMax = tyBE + barraAltura / 2;
    ...
    // Verifica colisão (retângulo com círculo)
    if (bolaX - bolaRaio <= barraX + barraLargura / 2 &&
        bolaX + bolaRaio >= barraX - barraLargura / 2 &&
        bolaY >= barraYMin &&
        bolaY <= barraYMax) {
        
        txBola_offset = -txBola_offset; // Inverte direção
        return true;
    }
    return false;
}

function verificaColisaoBarraDireita() {
    // Posição da barra direita
    const barraX = 0.9;
    const barraYMin = tyBD - barraAltura / 2;
    const barraYMax = tyBD + barraAltura / 2;
...
function atualizaAnimacao(){
    ...
    // Movimento da barra esquerda (WASD)
    ...
    // Limitar barra esquerda dentro da tela
    if (tyBE > 1.0 - barraAltura / 2) tyBE = 1.0 - barraAltura / 2;
    if (tyBE < -1.0 + barraAltura / 2) tyBE = -1.0 + barraAltura / 2;
    ...
    // Limitar barra direita dentro da tela
    if (tyBD > 1.0 - barraAltura / 2) tyBD = 1.0 - barraAltura / 2;
    if (tyBD < -1.0 + barraAltura / 2) tyBD = -1.0 + barraAltura / 2;

    // Verificar colisões com as barras
    verificaColisaoBarraEsquerda();
    verificaColisaoBarraDireita();

    // Atualizar posições das barras
